    ...
    def post(self, request):
        ...
        if form_type == 'calculator':
            ...
            if form.is_valid():
                ...
                recipients = ['info@yourcompany.com']
                
                send_mail(subject, message, sender, recipients, phone)
                messages.success(request, "Thank you! Your message has been sent successfully.")
                return redirect(reverse('index') + '#contact')
            else:
                messages.error(request, "Please correct the errors below.")
                # Ensure to pass back other forms or context if they are being displayed on the same page
                return render(request, 'calculator/index.html', {'form': form})

        return render(request, 'calculator/index.html', {'form': ContactForm()})
